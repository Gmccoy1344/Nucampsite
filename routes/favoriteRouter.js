...
.put(authenticate.verifyUser, cors.corsWithOptions, (req, res) => {
    res.statusCode = 403;
...
.options(authenticate.verifyUser, cors.corsWithOptions, (req, res) => res.sendStatus(200))
.get(cors.cors, (req, res, next) => {
    res.statusCode = 403;
    res.end('GET operation not supported on /favorites/campsites');
})
.post(authenticate.verifyUser, cors.corsWithOptions, (req, res, next) => {
    Favorite.findOne({user: req.user._id})
    .then(favorites => {
...
.put(authenticate.verifyUser, cors.corsWithOptions, (req, res, next) => {
    res.statusCode = 403;
    res.end('PUT operation not supported on /favorites/campsites');
})
.delete(authenticate.verifyUser, cors.corsWithOptions, (req, res, next) => {
    Favorite.findOne({user: req.user._id})
    .then(favorite => {
